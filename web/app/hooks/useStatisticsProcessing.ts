    ...
    const processed_data = useMemo(() => {
        ...
        for (const item of filtered) {
            ...
            if (!aggregatedMap.has(key)) {
                ...
                // In local benchmarks, this reduces iteration time significantly for large datasets.
        ...
        for (const item of results) {
            ...
            if (mode !== "all") {
                ...
                if (breakdown) {
                    // ⚡ Bolt Performance Optimization:
                    // Replaced `Object.values().reduce()` with `Object.keys()` and a for loop
                    // to avoid intermediate array allocations in this high-frequency calculation path.
