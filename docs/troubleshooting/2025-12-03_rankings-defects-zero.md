...
However, the live site still showed incorrect data because:

- GitHub Actions cache key `processed-data-v2-2025-W49` contained the old data
- Stage 3 restored from this stale cache instead of using the freshly generated artifact

## Resolution (2025-12-04)

Bumped the cache key version from `v2` to `v3` in:

- `.github/workflows/data_process.yml`
- `.github/workflows/website_build.yml`

This forced a cache miss, so Stage 3 used the freshly generated artifact.

## Verification

After cache bust, confirmed via curl with cache bypass:

- `rankings.json` now contains `defects_per_year` field
- Values are correct (e.g., Morgan at 0.0154, Ferrari at 0.0158)
